...
def get_stats():
    with SessionContext() as session:
        ...
        by_status = dict(session.execute(select(Loan.status, func.count()).group_by(Loan.status)).all())
        by_currency = dict(session.execute(select(Loan.currency, func.count()).group_by(Loan.currency)).all())
